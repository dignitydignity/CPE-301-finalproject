    ...
    case 'I':
      ...
      *portH &= ~0x02;
      ...
      if (water < waterThresh) {
        ...
        char buffer[32];
        int len = sprintf(buffer, "%d:%d:%d: LOW WATER ERROR", stamp.hour(), stamp.minute(), stamp.second());
        for (int i = 0; i < len; i++) {
          putchar(buffer[i]);
        }
        putchar('\n');
      ...
      if (temp > tempThresh) {
        ...
        DateTime stamp = rtc.now();
        char buffer[32];
        int len = sprintf(buffer, "%d:%d:%d: HI TEMP FAN ON", stamp.hour(), stamp.minute(), stamp.second());
        for (int i = 0; i < len; i++) {
          putchar(buffer[i]);
        }
        putchar('\n');
    ...
    *portE &= ~0x02;
      ...
      *portH &= ~0x02;
      ...
      if (~(*pinD & 0x01) && water > waterThresh) {
        ...
        DateTime stamp = rtc.now();
        char buffer[32];
        int len = sprintf(buffer, "%d:%d:%d: REFILLED RESTARTING", stamp.hour(), stamp.minute(), stamp.second());
        for (int i = 0; i < len; i++) {
          putchar(buffer[i]);
        }
        putchar('\n');
